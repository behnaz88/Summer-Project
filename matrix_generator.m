function [f, Aineq,bineq,Aeq, beq, sostype, sosind, soswt, lb, ub, ctype] = matrix_generator(run_duration, buffer_capacity, x0)
    ...
    for i = 1 : n
        ...
        elseif(slopes(i)<0)
            ...
            Aeq(n + i + 1, 6 * n + i + 1) = 1;     %deltaZ_i-
            
            Aeq(3 * n + i + 1, i) = -1 / slopes(i);   %%%%%%%%%
            Aeq(3 * n + i + 1, 2 * n + i + 1) = 1;
            Aeq(3 * n + i + 1, 5 * n + i + 1) = -1;
            
            beq(n + i + 1,1) = critical_time(i+1)-critical_time(i) - (buffer_capacity/slopes(i));
            beq(i+1,1) = (critical_time(i+1)-critical_time(i)) * slopes(i);
            
            beq(3 * n + i + 1, 1) = critical_time(i+1) - critical_time(i) - (buffer_capacity/slopes(i));
         elseif(slopes(i)==0)
