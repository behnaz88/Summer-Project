function [f, Aeq, beq,Aineq,bineq, lb, ub, simulation_results, mathematical_results] = matrix_generator_without_ineq_b(run_duration, buffer_capacity, x0)

    %Create Random States
    [critical_time, slopes, cop, cop, cop, cop, cop, cop, simulation_results, mathematical_results] = Input2(run_duration, 1.2, 1, buffer_capacity);
    n = size(slopes, 2);

    %Objective Function
    f = [zeros(3 * n + 1, 1);ones(4 * n, 1)];

    %Equality Matrices
    Aeq = zeros(3 * n + 1, 7 * n + 1);
    beq = zeros(3 * n + 1, 1);
